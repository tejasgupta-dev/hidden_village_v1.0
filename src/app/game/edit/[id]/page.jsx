...
export default function GameEditor({ isNew = false }) {
  ...
  const id = pathname.split("/").filter(Boolean).pop(); // safer for trailing slashes
  ...
  return (
    <div className="min-h-screen bg-transparent py-4 px-3">
      <div className="max-w-5xl mx-auto space-y-4">
        <Header user={user} game={game} isNew={isNew} handleBack={handleBack} />
        ...
        <LevelEditor
          ...
          allAvailableLevels={allAvailableLevels}
          showAddLevel={showAddLevel}
          setShowAddLevel={setShowAddLevel}
          addLevel={addLevel}
...
/* --------------------------
   HEADER
-------------------------- */
function Header({ user, game, isNew, handleBack }) {
  return (
...
/* --------------------------
   ACTION BUTTONS
-------------------------- */
function ActionButtons({ savingGame, isNew, handleSave, handleDelete, handleBack }) {
  return (
